...
class QueryBubbleSize extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isValid: "",
  ...
  componentDidMount() {
    ...
    const queryBubbleSize = this.props.queryBubbleSize;
    if (isNotEmpty(chartId) && Object.keys(queryBubbleSize).length > 0) {
      const savedQueryBubbleSize = queryBubbleSize;
      let filteredItems = [];
      filteredItems.push(savedQueryBubbleSize);
      ...
      let bubbleSize = {
        value: savedQueryBubbleSize["column"],
        label: this.savedMetricsComponent(savedQueryBubbleSize),
      };
      this.setState({ defaultQueryBubbleSize: bubbleSize });
    }
  }
    ...
    return (
      <div role="button" tabIndex="0">
        <button
          ...
          id={"bubble-size-" + element["columnName"].toLowerCase()}
  ...
  metricsModalSave() {
    ...
    this.props.setQueryBubbleSize(queryFilterOption);
    ...
    document.getElementById("bubble-size-" + queryMetricsCurrItemName).value =
      modifiedBtnText;
    document.getElementById(
      "bubble-size-" + queryMetricsCurrItemName
    ).innerText = modifiedBtnText;
  ...
  handleInputChangeMetricsOptions = (selectedOption) => {
    //
    this.setState({ defaultQueryBubbleSize: selectedOption });
  };
  ...
  render() {
    const renderInfoQueryBubbleSize = (props) => (
    ...
    return (
      <div className="row">
        <div className="col" style={{ paddingBottom: 10 }}>
            ...
            >
              ...
              >
                <OverlayTrigger
                  ...
                  overlay={renderInfoQueryBubbleSize}
                >
          ...
          <div className="row" style={{ paddingLeft: 20 }}>
            <AsyncSelect
              value={this.state.defaultQueryBubbleSize}
              cacheOptions
              defaultOptions
...
const mapDispatchToProps = (dispatch) => ({
  ...
  setQueryBubbleSize: (data) => dispatch(action.setQueryBubbleSize(data)),
});

export default connect(mapStateToProps, mapDispatchToProps)(QueryBubbleSize);
