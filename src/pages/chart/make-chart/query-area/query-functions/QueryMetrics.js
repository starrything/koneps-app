...
class QueryMetrics extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isValid: "",
  ...
  componentDidMount() {
    ...
    console.log("queryMetrics - componentDidMount()");
    console.log("chartId: " + this.props.chartId);
    console.log("datasetSpecs: " + datasetSpecs);
    console.log("savedQueryMetrics: " + this.props.queryMetrics);
    if (isNotEmpty(this.props.chartId) && datasetSpecs.length > 0) {
      const savedQueryMetrics = this.props.queryMetrics;
      let filteredItems = [];
      datasetSpecs.forEach((element) => {
        // element : all elements from react-select List
        let filteredItem = savedQueryMetrics.filter(
          (metric) => element["columnName"].toLowerCase() === metric.column.toLowerCase()
        );
      ...
      this.setState({ queryMetricsToStore: savedQueryMetrics });
      this.setState({ defaultQueryMetrics: filteredList });
    }
  }
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.datasetSpecs !== this.props.datasetSpecs) {
      ...
      console.log("queryMetrics - componentDidUpdate()");
      console.log("chartId: " + this.props.chartId);
      console.log("datasetSpecs: " + datasetSpecs);
      console.log("savedQueryMetrics: " + this.props.queryMetrics);
      if (isNotEmpty(this.props.chartId) && datasetSpecs.length > 0) {
        const savedQueryMetrics = this.props.queryMetrics;
        let filteredItems = [];
        console.log("Query Metrics - filtering...");
        datasetSpecs.forEach((element) => {
          // element : all elements from react-select List
          let filteredItem = savedQueryMetrics.filter(
            (metric) => element["columnName"].toLowerCase() === metric.column.toLowerCase()
    ...
    return (
      <div role="button" tabIndex="0">
        <button
          ...
          id={"metrics-" + element["columnName"].toLowerCase()}
  ...
  metricsModalSave() {
    ...
    let queryMetricsToStore = this.state.queryMetricsToStore;
    let queryMetricsSequence = queryMetricsToStore.length;
    let queryMetricsOptions = {};
    queryMetricsOptions = {
      key: queryMetricsSequence,
      column: queryMetricsCurrItemName,
      metrics: queryMetricsCodeKey,
    };
    queryMetricsToStore.push(queryMetricsOptions);
    this.setState({ queryMetricsToStore: queryMetricsToStore });
    this.props.setQueryMetrics(queryMetricsToStore);
    ...
    document.getElementById("metrics-" + queryMetricsCurrItemName).value =
      modifiedBtnText;
    document.getElementById("metrics-" + queryMetricsCurrItemName).innerText =
      modifiedBtnText;
  ...
  handleInputChangeMetricsOptions = (selectedOption) => {
    ...
    if (queryMetricsToStore.length > 0) {
      if (selectedOption.length > 0) {
        console.log("Query.Metrics Not Null");
        selectedOption.forEach((el, key) => {
          let columnName = el["value"].toLowerCase();

          let findArray = queryMetricsToStore.filter((element) =>
        ...
        this.setState({ queryMetricsToStore: modifiedQueryFilterList });
        this.props.setQueryMetrics(modifiedQueryFilterList);
        this.setState({ defaultQueryMetrics: selectedOption });
      } else {
        console.log("Query.Metrics IS NULL");
      ...
      console.log("Query.Metrics Not Null");
        selectedOption.forEach((el, key) => {
          let columnName = el["value"];

          let findArray = queryMetricsToStore.filter((element) =>
            element["column"].match(columnName)
          );
  ...
  render() {
    const renderInfoQueryMetrics = (props) => (
    ...
    return (
      <div className="row">
        <div className="col" style={{ paddingBottom: 10 }}>
            ...
            >
              ...
              >
                <OverlayTrigger
                  ...
                  overlay={renderInfoQueryMetrics}
                >
          ...
          <div className="row" style={{ paddingLeft: 20 }}>
            <AsyncSelect
              value={this.state.defaultQueryMetrics}
              isMulti
              cacheOptions
              defaultOptions
...
const mapDispatchToProps = (dispatch) => ({
  ...
  setQueryMetrics: (data) => dispatch(action.setQueryMetrics(data)),
});

export default connect(mapStateToProps, mapDispatchToProps)(QueryMetrics);
