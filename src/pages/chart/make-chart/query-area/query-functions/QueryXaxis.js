...
class QueryXaxis extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isValid: "",
  ...
  componentDidMount() {
    ...
    const queryXaxis = this.props.queryXaxis;
    if (isNotEmpty(chartId) && Object.keys(queryXaxis).length > 0) {
      const savedQueryXaxis = queryXaxis;
      let filteredItems = [];
      filteredItems.push(savedQueryXaxis);
      ...
      let xAxis = {
        value: savedQueryXaxis["column"],
        label: this.savedMetricsComponent(savedQueryXaxis),
      };
      this.setState({ defaultQueryXaxis: xAxis });
    }
  }
    ...
    return (
      <div role="button" tabIndex="0">
        <button
          ...
          id={"xaxis-" + element["columnName"].toLowerCase()}
  ...
  metricsModalSave() {
    ...
    this.props.setQueryXaxis(queryFilterOption);
    ...
    document.getElementById("xaxis-" + queryMetricsCurrItemName).value =
      modifiedBtnText;
    document.getElementById("xaxis-" + queryMetricsCurrItemName).innerText =
      modifiedBtnText;
  ...
  handleInputChangeMetricsOptions = (selectedOption) => {
    //
    this.setState({ defaultQueryXaxis: selectedOption });
  };
  ...
  render() {
    const renderInfoQueryXaxis = (props) => (
    ...
    return (
      <div className="row">
        <div className="col" style={{ paddingBottom: 10 }}>
          ...
          <div className="row" style={{ paddingLeft: 20 }}>
            <AsyncSelect
              value={this.state.defaultQueryXaxis}
              cacheOptions
              defaultOptions
...
const mapDispatchToProps = (dispatch) => ({
  ...
  setQueryXaxis: (data) => dispatch(action.setQueryXaxis(data)),
});

export default connect(mapStateToProps, mapDispatchToProps)(QueryXaxis);
